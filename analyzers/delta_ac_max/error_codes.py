...
ERROR_CODES = {
    ...
    'EV0105': {'desc': 'Over Phase Input', 'cause': 'Single-phase model connected to 3-phase input voltage', 'fix': 'Power off charger and verify input line wiring (remove L2/L3 for single-phase model)'},
    'EV0106': {'desc': 'Lack of Current', 'cause': 'Output current cannot be detected or is detected incorrectly (current feedback abnormal)', 'fix': 'Power cycle charger, verify current feedback path, service hardware if feedback remains incorrect'},
    'EV0107': {'desc': 'Output Connector OTP', 'cause': 'Output connector temperature over protection threshold', 'fix': 'Power off 10min and retest; inspect connector/cable temperature path if persists'},
    'EV0108': {'desc': 'Relay OTP', 'cause': 'Relay temperature over protection threshold', 'fix': 'Power off 10min and retest; inspect relay thermal condition if persists'},
    'EV0109': {'desc': 'PEN Fault', 'cause': 'N-PE voltage over 70V', 'fix': 'Check grounding and PE wiring; service site electrical installation if persists'},
